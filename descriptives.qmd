...
```{r loadpacksdist4, include = F, cache=FALSE}
source("loader.R")
```

When we encounter large lists of unsorted numbers, we have no natural capacity to perceive the list's fundamental characteristics such as its average or its variability. Therefore, we need [descriptive statistics]{.defword}[**Descriptive statistics** summarize noteworthy characteristics of samples.]{.defword .column-margin} to quantify salient characteristics of distributions, or we need to display the numbers in a plot, making them easier to understand and communicate.

## Frequency Distribution Tables

A simple way to describe a distribution is to list how many times each value in the distribution occurs. For example, in this distribution: $\{10, 3, 4, 10, 6, 4, 6, 4\}$, there is 1 three, 3 fours, 2 sixes, and 2 tens. The value that occurs most often is four. A [frequency distribution table]{.defword}[A **frequency distribution table** summarises a sample by showing the frequency counts of each member of the sample space.]{.defword .column-margin} displays the number of times each value occurs, as in @tbl-frequencydistributiontable.
...
```{r frequencydistributiontablCode}
#| button_r: true
#| figlabel: tbl-frequencydistributiontable
...
It is also common to supplement frequency distribution tables with additional information such as the [cumulative frequency]{.defword}[The **cumulative frequency** tells us the number of scores in a distribution that are equal to or lower than a particular sample space element.]{.defword .column-margin}. For each sample space element, the cumulative frequency $(cf)$ is the sum of the frequencies $(f)$ of the current and all previous sample space elements.
...
```{r freqtablepubCode}
#| button_r: true
#| figlabel: tbl-freqtablepub
<<tbl-freqtablepub>>
```

### Frequency Distribution Bar Plots

```{r fig-freqbarplot, opts.label="marginfigure"}
#| fig-height: 7
...
```{r fig-freqstackedbarplot, opts.label="marginfigure"}
#| fig-width: 2.5
...
```{r fig-freqstepplot, opts.label="marginfigure"}
...
```{r freqstackedbarplotCode}
#| button_r: true
#| figlabel: fig-freqstackedbarplot
...
The [mode]{.defword}[The **mode** is the value in a distribution that occurs most often.]{.defword .column-margin} is the most frequent score in a distribution. Suppose we have a distribution that looks like this: 
...
If two values tie, both values are the mode and the distribution is [bimodal]{.defword}[A **bimodal** distribution has two modes.]{.defword .column-margin}. Sometimes a distribution has two distinct clusters, each with its own local mode. The greater of these two modes is the *major mode*, and the lesser is the *minor mode* (See @fig-bimodal).

```{r fig-bimodal, opts.label="marginfigure"}
...
The [arithmetic mean]{.defword}[The **arithmetic mean** is the balance point of a disribution.]{.defword .column-margin} is the sum of all values of a distribution divided by the size of the distribution.
...
```{r centraltendencyCode}
#| button_r: true
#| figlabel: fig-centraltendency
<<fig-centraltendency>>
```
...
At one level, the concept of the [expected value]{.defword}[The **expected value** of a random variable is the population mean of the values that the random variable generates.]{.defword .column-margin} of a random variable is really simple; it is just the population mean of the variable. So why don't we just talk about population means and be done with this "expected value" business? It just complicates things! True. In this case, however, there is value in letting some simple things appear to become complicated for a while so that later we can show that some apparently complicated things are actually simple.
...
```{r fig-pmfX, opts.label="marginfigure"}
...
```{r fig-thinbins, opts.label="marginfigure"}
#| fig-width: 5
#| fig-height: 7
...
```{r fig-nominalvar, opts.label="marginfigure"}
...
```{r fig-ordinalIQR, opts.label="marginfigure"}
...
```{r IQRboxplotCode}
#| button_r: true
#| figlabel: fig-IQRboxplot
<<fig-IQRboxplot>>
```

### Variance 

A [deviation]{.defword}[A **deviation** is the distance of a score from the score's mean.]{.defword .column-margin} is computed by subtracting a score from its mean:
...
[Variance]{.defword}[**Variance** is a measure of variability that gives the size of the average squared deviation from the mean.]{.defword .column-margin} $\left(\sigma^2\right)$ is the expected value of squared deviations from the mean $\left(\mu\right)$:
...
```{r fig-besselpic, opts.label="marginfigure"}
...
```{r variancevisualizationCode}
#| button_r: true
#| figlabel: fig-variancevisualization
...
The [standard deviation]{.defword}[The **standard deviation** is a measure of variabily that estimates the typical distance of a score from its own mean.]{.defword .column-margin} is by far the most common measure of variability. The standard deviation $\sigma$ is the square root of the variance $\sigma^2$.
...
In a normal distribution, the standard deviation is the distance from the mean to the two [inflection points]{.defword}[An **inflection point** in a curve is the point at which the curvature changes from upward to downward or vice versa.]{.defword .column-margin} in the probability density curve (see @fig-normalinflection).
...
```{r normalinflectionCode}
#| button_r: true
#| figlabel: fig-normalinflection
...
A [central moment]{.defword}[A **central moment** is a raw moment after the variable's mean has been subracted.]{.defword .column-margin} $\mu_n$ is a moment about the mean (i.e., the first raw moment):
