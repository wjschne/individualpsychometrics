...
tibble(x = seq(0,20,.1)) |>
  ...
  geom_text(data = tibble(x = c(2.2,10, 16), y = .08, label = c(paste("Lognormal\nSkewness = ", round(distributional::dist_lognormal(1,1) %>% distributional::skewness(),1)), "Normal", "Weibull")), aes(x = x, y = y, label = label), family = bfont, size = bsize, size.unit = "pt") +
  scale_fill_manual(
    "Distribution",
    values = c(Normal = myfills[3], 
               Lognormal = myfills[2], 
               Weibull = myfills[1])
  ) +
  scale_y_continuous(NULL, breaks = NULL, expand = expansion()) +
