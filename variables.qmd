...
Because there are so many different kinds of [variables]{.defword}[A **variable** is an abstract category of information that can take on different values.]{.column-margin .defword}, it is helpful to classify them by type so that we can think clearly about them and not use them incorrectly. Some statistics only make sense with certain types of variables. To take a famously silly example from @zachariasTroubleIQTests1975, you could calculate the average number in a telephone directory, but you wouldn't try calling it. The mean has no meaning for this kind of variable. Unfortunately, it is not always so obvious which statistics can be applied to which variables. If we unknowingly apply the wrong kind of statistic to a variable, our statistical software will give us a result, *but we will not know that it is nonsense*.

The classic [taxonomy]{.defword}[A **taxonomy** is a classification system.]{.column-margin .defword} for variable types [@stevensTheoryScalesMeasurement1946] distinguishes among nominal, ordinal, interval, and ratio scales (@fig-Stevens). Although it is clear that this taxonomy is incomplete [@cicchetti2006rating], there is no consensus about how exactly the list should be amended. If this book were about how to design psychological tests, we would explore this topic in depth. For our purposes, it is enough to say that the fundamentals of measurement and [scaling]{.defword}[**Scaling** refers to the procedures by which we decide how to assign numbers to the attributes we measure.]{.column-margin .defword} for variable types is philosophically complex and that controversies abound [@michellQuantitativeScienceDefinition1997].
...
#| figlabel: fig-Stevens

```

## Nominal Scales {#nominal}

In a [nominal scale]{.defword}[A **nominal scale** groups observations into unordered categories.]{.column-margin .defword}, we note only that some things are different from others and that they belong to two or more [mutually exclusive]{.defword}[In **mutually exclusive** categories nothing belongs to more than one category (at the same time and in the same sense).]{.column-margin .defword} categories. If we say that a person has Down syndrome (trisomy 21), we are implicitly using a nominal scale in which there are people with Down syndrome and people without Down syndrome (as in @fig-dichotomy). In a true nominal scale, there are no cases that fall between categories. To be sure, we might have some difficulty figuring out and reliably agreeing upon the category to which something belongs---but there is no conceptual space between categories.

```{r fig-dichotomy, opts.label="marginfigure"}
#| fig-cap: "A (nearly) true dichotomy"
#| fig-align: center
knitr::include_graphics("images/dichotomy.svg") 
```

```{tikz dichotomyCode}
#| button_latex: true
#| code: !expr 'readLines("tex/dichotomy.tex")'
#| figlabel: fig-dichotomy
```
...
```{tikz LikertCode}
#| button_latex: true
#| code: !expr 'readLines("tex/Likert.tex")'
#| figlabel: fig-Likert
```
...
[^variables-1]: Obviously, some of these degrees can be skipped and the endpoint is different for different careers in psychology. Furthermore, not all degrees fit neatly in this sequence (e.g., the school psychology specialist degree).
...
#| fig-height: 6
#| out-height: "600"
knitr::include_graphics("images/SUDS.svg")

```

```{tikz sudsCode}
#| button_latex: true
#| code: !expr 'readLines("tex/SUDS.tex")'
#| figlabel: fig-suds
```
...
In a [ratio scale]{.defword}[A **ratio scale** has a true zero, in addition to all the properties of an interval scale.]{.column-margin .defword}, zero represents something special: the absence of the quantity being measured. In an interval scale, there may be a zero, but the zero is just another number in the scale. For example, 0°C happens to be the freezing point of water at sea level but it does not represent the absence of heat.^[The absence of heat occurs at −273.15°C or 0°K.] Ratio scales do not usually have negative numbers, but there are exceptions. For example, in a checking account balance, negative numbers indicate that the account is overdrawn. Still, a checking account balance is a true ratio scale because a zero indicates that there is no money in the account.
...
knitr::include_graphics("images/DiscreteContinuous.svg")
```

```{tikz DiscreteContinuousCode}
#| button_latex: true
#| code: !expr 'readLines("tex/DiscreteContinuous.tex")'
#| figlabel: fig-DiscreteContinuous
```
