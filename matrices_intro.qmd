...
### Testing if a matrix is diagonal

The Matrix package has a function `isDiagonal` that will test if a matrix is diagonal.

```{r}
library(Matrix)
isDiagonal(matrix(c(1, 2, 3, 4), nrow = 2))
isDiagonal(matrix(c(1, 0, 0, 2), nrow = 2))
```
