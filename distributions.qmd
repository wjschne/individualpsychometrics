...
```{r loadpacksdist3, include = F, eval=TRUE, cache=FALSE}
source("loader.R")
```
...
[Random variables]{.defword}[**Random variables** have values that are determined by a random process.]{.column-margin .defword} are not like algebraic variables. Random variables simply take on values because of some random process. If we say that the outcome of a throw of a six-sided die is a random variable, there is nothing to "solve for." There is no equation that determines the value of the die. Instead, it is determined by chance and the physical constraints of the die. That is, the outcome must be one of the numbers printed on the die, and the six numbers are equally likely to occur. This illustrates an important point. The word *random* here does not mean "anything can happen." On a six-sided die, you will never roll a 7, 3.5, $\sqrt{\pi}$, &minus;36,000, or any other number that does not appear on the six sides of the die. Random variables have outcomes that are subject to random processes, but those random processes *do*  have constraints on them such that some outcomes are more likely than others---and some outcomes never occur at all. 

```{r fig-randomvariable, opts.label="marginfigure"}
#| fig-cap: "Rolling a six-sided die is a process that creates a randomly ordered series of integers from 1 to 6."
#| classes: moveup300
#| out-width: "100"
knitr::include_graphics("images/randomvariable.gif")
```
...
```{r randomvariableCode}
#| button_r: true
#| figlabel: fig-randomvariable
...
A [set]{.defword}[A **set** is a collection of distinct objects.]{.column-margin .defword} refers to a collection of objects. Each distinct object in a set is an [element]{.defword}[An **element** is a distinct member of a set.]{.column-margin .defword}. 

### Discrete Sets

To show that a list of discrete elements is a [discrete set]{.defword}[A **discrete set** has numbers that are isolated, meaning that each number has a range in which it is the only number in the overall set.]{.column-margin .defword}, we can use curly braces. For example, the set of positive single-digit even numbers is $\{2, 4, 6, 8\}$. With large sets with repeating patterns, it is convenient to use an ellipsis ("..."), the punctuation mark signifying an omission or pause. For example, rather than listing every two-digit positive even number, we can show the pattern like so: 
...
With continuous variables, we can define sets in terms of [intervals]{.defword}[**Intervals** are a continous range of numbers.]{.column-margin .defword}. Whereas the discrete set $\{0,1\}$ refers just to the numbers 0 and 1, the interval set $(0,1)$ refers to all the numbers between 0 and 1. 

```{r fig-intervalnotation, opts.label="marginfigure"}
...
```{r intervalnotationCode}
#| button_r: true
#| figlabel: fig-intervalnotation
...
The set of all possible outcomes of a random variable is the [sample space]{.defword}[A **sample space** is the set of all possible values that a random variable can assume.]{.column-margin .defword}. Continuing with our example, the sample space of a single throw of a six-sided die is the set $\{1,2,3,4,5,6\}$. *Sample space* is a curious term. Why *sample* and why *space*? With random variables, [populations]{.defword}[A **population** consists of all entities under consideration.]{.column-margin .defword} are infinitely large, at least theoretically. Random variables just keep spitting out numbers forever! So any time we actually observe numbers generated by a random variable, we are always observing a [sample]{.defword}[A **sample** is a subset of a population.]{.column-margin .defword}; actual infinities cannot be observed in their entirety. A *space* is a set that has mathematical structure. Most random variables generate either integers or real numbers, both of which are structured in many ways (e.g., order).
...
Each element of a random variable's sample space occurs with a particular probability. When we list the probabilities of each possible outcome, we have specified the variable's [probability distribution]{.defword}[In a **probability distribution**, there is an assignment of a probability to each possible element in a variable's sample space.]{.column-margin .defword}. In other words, if we know the probability distribution of a variable, we know how probable each outcome is. In the case of a throw of a single die, each outcome is equally likely (@fig-dice).

```{r fig-dice, opts.label="marginfigure"}
#| fig-cap: "The probability distribution of a throw of a single die"
#| out-width: "275"
#| fig-width: 6.72
#| fig-height: 4
#| classes: moveup100
knitr::include_graphics("images/dice.svg")
```

```{tikz DicePMF}
#| button_latex: true
#| figlabel: fig-dice
#| code: !expr 'readLines("tex/dice.tex")'
```
...
```{r pdfIllustrationCode}
#| button_r: true
#| figlabel: fig-pdfIllustration
...
The throw of a single die is a member of a family of distributions called the [discrete uniform distribution]{.defword}[A **discrete uniform distribution** is a family of random variable distributions in which the sample space is an evenly spaced sequence of numbers, each of which is equally likely to occur.]{.column-margin .defword}. It is "discrete" because the elements in the sample space are countable, with evenly spaced gaps between them. For example, there might be a sequence of 8, 9, 10, and 11 in the sample space but there are no numbers in between. It is "uniform" because all outcomes are equally likely. With dice, the numbers range from a lower bound of 1 to an upper bound of 6. In the family of discrete uniform distributions, the lower and upper bounds are typically integers, mostly likely starting with 1. However, any real number $a$ can be the lower bound and the spacing $k$ between numbers can be any positive real number. For the sake of simplicity and convenience, I will assume that the discrete uniform distribution refers to consecutive integers ranging from a lower bound of $a$ and an upper bound of $b$. 
...
```{r tbl-uniformfeatures, echo = F, cache = T}
...
The lower bound $a$ and the upper bound $b$ are the discrete uniform distribution's [parameters]{.defword}[A **parameter** is a defining feature of a random variable's probability distribution.]{.column-margin .defword}. The word *parameter* has many meanings, but here it refers to a characteristic of a distribution family that helps us identify precisely which member of the family we are talking about. Most distribution families have one, two, or three parameters. 
...
Many distribution families are united by the fact that their probability distributions are generated by a particular mathematical function. For discrete distributions, those functions are called [probability mass functions]{.defword}[A **probability mass function** is a mathematical expression that gives the probability that a discrete random variable will equal a particular element of the variable's sample space.]{.column-margin .defword}. In general, a mathematical function is an expression that takes one or more constants (i.e., parameters) and one or more input variables, which are then transformed according to some sort of rule to yield a single number.
...
knitr::include_graphics("images/pmf.svg")
```

```{tikz pmfCode}
#| button_latex: true
#| figlabel: fig-pmf
#| code: !expr 'readLines("tex/pmf.tex")'
```
...
The [cumulative distribution function]{.defword}[A **cumulative distribution function** is a mathematical expression that gives the probability that a random variable will equal a particular element of the variable's sample space or less.]{.column-margin .defword} tells us where a sample space element ranks in a distribution. Whereas the probability mass function tells us the probability that a random variable will generate a particular number, the cumulative distribution function tells us the probability that a random variable will generate a particular number or less. 
...
#| button_latex: true
#| figlabel: fig-cdfDie
#| code: !expr 'readLines("tex/cdfDie.tex")'
```
...
The inverse of the cumulative distribution function is the [quantile function]{.defword}[A **quantile function** tells us which value in the sample space of a random variable is greater than a particular proportion of the values the random variable generates.]{.column-margin .defword}. The cumulative distribution starts with a value $x$ in the sample space and tells us $p$, the proportion of values in that distribution that are less than or equal to $x$. A quantile function starts with a proportion $p$ and tells us the value $x$ that splits the distribution such that the proportion $p$ of the distribution is less than or equal to $x$. 
...
```{r fig-discreteplot, opts.label="marginfigure"}
#| fig-asp: 1
...
The toss of a single coin has the simplest probability distribution that I can think of---there are only two outcomes and each outcome is equally probable (@fig-coin). This is a special case of the [Bernoulli distribution]{.defword}[In the **Bernoulli distribution**, there are only two outcomes: a &ldquo;success&rdquo; (1) and a &ldquo;failure&rdquo; (0). If a success has a probability $p$ then a failure has a probability of $q = 1 - p$.]{.column-margin .defword}. Jakob Bernoulli (@fig-bernoullipic) was a famous mathematician from a famous family of mathematicians. The Bernoulli distribution is just one of the ideas that made Jakob and the other Bernoullis famous. 

```{tikz coinCode}
#| button_latex: true
#| figlabel: fig-coin
#| code: !expr 'readLines("tex/coin.tex")'
```

```{r fig-coin, opts.label="marginfigure"}
#| out-width: "300"
#| fig-cap: "The probability distribution of a coin toss"
knitr::include_graphics("images/coin.svg")
```

```{r fig-bernoullipic, opts.label="marginfigure"}
...
```{r fig-BernoulliSample, opts.label="marginfigure"}
...
```{r BernoulliSampleCodeFigure}
#| button_r: true
#| figlabel: fig-BernoulliSample
<<fig-BernoulliSample>>
```
...
```{tikz twocoinCode}
#| button_latex: true
#| figlabel: fig-twocoin
#| code: !expr 'readLines("tex/twocoin.tex")'
```


The probability distribution of the number of heads observed when two coins are tossed at the same time is a member of the *binomial distribution* family. The binomial distribution occurs when [independent]{.defword}[Two random variable are said to be **independent** if the outcome of one variable does not alter the probability of any outcome in the other variable.]{.column-margin .defword} random variables with the same [Bernoulli distribution](#sec:BernoulliDist) are added together. In fact, Bernoulli discovered the binomial distribution as well as the Bernoulli distribution.

Imagine that a die is rolled 10 times and we count how often a 6 occurs.^[Wait! Hold on! I thought that throwing dice resulted in a (discrete) *uniform* distribution. Well, it still does. However, now we are asking a different question. We are only concerned with two outcomes each time the die is thrown: 6 and not 6. This is a Bernoulli distribution, not a uniform distribution, because the probability of the two events is unequal: {&frac16;,&frac56;}] Each roll of the die is called a [trial]{.defword}[Every time a random variable generates a number, that instance of the variable is called a **trial**, which is also known as an **experiment**.]{.column-margin .defword}. The sample space of this random variable is $\{0,1,2,...,10\}$. What is the probability that a 6 will occur 5 times? or 1 time? or not at all? Such questions are answered by the binomial distribution's [probability mass function](#sec:pmf):
...
```{r fig-pmf6, opts.label="marginfigure"}
...
When would a binomial distribution be used by a clinician? One particularly important use of the binomial distribution is in the detection of [malingering]{.defword}[A person who **malingers** is pretending to be sick to avoid work or some other responsibility.]{.column-margin .defword}. Sometimes people pretend to have memory loss or attention problems in order to win a lawsuit or collect insurance benefits. There are a number of ways to detect malingering but a common method is to give a very easy test of memory in which the person has at least a 50\% chance of getting each test item correct even if the person guesses randomly. 
...
```{r BinomialDistributionCode}
#| button_r: true
#| figlabel: fig-BinomialDistribution
...
```{r fig-PoissonPortrait, opts.label="marginfigure"}
...
Imagine that an event happens sporadically at random and we measure how often it occurs in regular time intervals (e.g., events per hour). Sometimes the event does not occur in the interval, sometimes just once, and sometimes more than once. However, we notice that over many intervals, the average number of events is constant. The distribution of the number of events in each interval will follow a [Poisson distribution]{.defword}[The **Poisson distribution** is a discrete distribution used to model how often an event will occur during a particular interval of time.]{.column-margin .defword}. Although "Poisson" means "fish" in French, fish have nothing to do with it. This distribution was named after Siméon Denis Poisson, whose work on the distribution made it famous.
...
```{r PanicFrequencyCode}
#| button_r: true
#| figlabel: fig-PanicFrequency
<<fig-PanicFrequency>>
```
...
```{r PanicCumulativeFrequencyCode}
#| button_r: true
#| figlabel: fig-PanicCumulativeFrequency
<<fig-PanicCumulativeFrequency>>
```

### Geometric Distributions

```{r tbl-GeometricFeatures, cache=T}
...
```{r WithMedicationCDFCode}
#| button_r: true
#| figlabel: fig-WithMedicationCDF
...
```{r fig-uniformdist, opts.label="marginfigure"}
...
```{r fig-uniformdistCode}
#| button_r: true
#| figlabel: fig-uniformdist
<<fig-uniformdist>>
```
...
```{r fig-GaussImage, opts.label="marginfigure"}
...
```{r tbl-tfeatures}
#| tbl-cap: "Features of *t* Distributions<br>**Notation note**: $\\Gamma$ is the gamma function. $_2F_1$ is the hypergeometric function."
#| tbl-cap-location: margin
#| tbl-colwidths: [50,50]
tribble(
  ~Feature, ~Symbol,
...
```{r fig-tnorm, opts.label="marginfigure"}
