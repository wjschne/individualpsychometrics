...
Most of us are comfortable with the notion of [probability]{.defword}[**Probability** is the ratio of the number outcomes in which the event of interest occurrs over the total number of outcomes.]{.defword .column-margin}, but we find [odds]{.defword}[**Odds** refers to the ratio of the number of outcomes in which the event occurs to the number of outcomes in which the event does not occur.]{.defword .column-margin} to be a little harder to understand. In everyday language, people use *probability* and *odds* interchangeably. Technically, they are related, but not at all the same thing.

When the odds of winning are 3 to 1, there will be 3 wins for every loss.

```{r, opts.label="marginfigure"}
#| label: fig-p2oddsfigure
...
```{r}
#| label: p2oddsfigurecode
#| button_r: true
#| figlabel: fig-p2oddsfigure
<<fig-p2oddsfigure>>
```
...
tibble(w = seq(450, 550),
  ...
  geom_richtext(data = . %>% 
                ...
                size = ggtext_size(18)) +
  scale_x_continuous("Ability – Difficulty (in W units)", breaks = seq(450, 550, 10), labels = \(x) paste0(signs::signs(x - 500), if_else(x >= 500, "", "\u2007"))) + 
...
The [Relative Proficiency Index (RPI)]{.defword}[The **Relative Proficiency Index (RPI)** tells us the proability a person with of ability $\\theta$ will answer an item correctly given that a person with ability $\\mu_\\theta$ has probability $p$ of answering it correctly.]{.defword .column-margin} was first used in the *Woodcock Reading Mastery Tests* [@woodcockWoodcockReadingMastery1973]. The RPI answers the following question:
...
```{r}
#| label: thresholdprobabilityCode
#| button_r: true
#| figlabel: fig-thresholdprobability
...
```{r}
#| label: multiplethresholdprobabilityCode
#| button_r: true
#| figlabel: fig-multiplethresholdprobability
